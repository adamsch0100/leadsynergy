...
import { Clock, Phone, RefreshCw, AlertTriangle, UserX } from "lucide-react"
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (leadsData.success && Array.isArray(leadsData.data)) {
        ...
        const unassignedLeads = leadsData.data.filter((l: Lead) => !l.assigned_user_id)
        setLeads(unassignedLeads)
        const sources = [...new Set(unassignedLeads.map((l: Lead) => l.source).filter(Boolean))]
    ...
    <SidebarWrapper role="admin">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground">
            Leads that need to be assigned to team members ({filteredLeads.length} leads)
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Leads waiting to be assigned to team members</CardDescription>
        </CardHeader>
        <CardContent>
