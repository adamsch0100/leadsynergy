...
class ToolExecutor:
    ...
    async def _execute_send_sms(
        ...
        # Truncate if too long (limit configured via settings, default 1000)
        max_len = getattr(self, 'max_sms_length', 1000)
        if len(message) > max_len:
            message = message[:max_len - 3] + "..."
            logger.warning(f"SMS truncated to {max_len} chars for person {fub_person_id}")
    ...
    async def _execute_send_email(
        ...
        """Send email to the lead via FUB web interface (Playwright).

        Sends the email directly through FUB's web UI so it appears in the
        lead's FUB timeline and comes from the agent's FUB email address.
        Falls back to creating a draft note if Playwright send fails.
        """
        ...
        # Try sending via Playwright (FUB web UI)
        try:
            ...
            if result.get("success"):
                logger.info(f"Email sent via Playwright for person {fub_person_id}: {subject}")
                return ExecutionResult(
                    success=True,
                    action="send_email",
                    ...
                    data={
                        ...
                        "sent_via": "playwright_fub",
                    },
                )
            else:
                logger.warning(
                    f"Playwright email send failed for person {fub_person_id}: {result.get('error')}. "
                    f"Falling back to draft note."
                )
        except Exception as e:
            logger.warning(
                f"Playwright email not available for person {fub_person_id}: {e}. "
                f"Falling back to draft note."
            )

        # Fallback: create a note with email draft + task for human review
        note_content = f"""
        <h3>AI Email (auto-send failed — please send manually)</h3>
        <p><strong>Subject:</strong> {subject}</p>
        <p><strong>Body:</strong></p>
        <div style="background: #f5f5f5; padding: 10px; border-radius: 4px;">
            {body}
        </div>
        <p><small>Playwright auto-send failed. Please copy this email and send it manually.</small></p>
        """

        note_result = self.note_service.post_note_to_person(
            person_id=fub_person_id,
            subject="AI Email Draft (needs manual send)",
            body=note_content,
            is_html=True,
        )
        ...
        task_result = self.sms_service.create_task(
            person_id=fub_person_id,
            description=f"SEND EMAIL to lead: {subject}",
        ...
        if note_result and "error" not in note_result:
            return ExecutionResult(
                success=False,
                action="send_email",
                message=f"Email draft created (auto-send failed): {subject}",
                error="Playwright send failed — draft saved as FUB note",
                data={
                    "subject": subject,
                    "body_preview": body[:100],
                    "sent_via": "draft_note_fallback",
            ...
            return ExecutionResult(
                success=False,
                action="send_email",
                message="Failed to send email and create draft",
    ...
    async def _execute_schedule_showing(
        ...
        sms_result = await self.sms_service.send_text_message_async(
            person_id=fub_person_id,
            message=message[:getattr(self, 'max_sms_length', 1000)],
            from_user_id=self.user_id,
        )
